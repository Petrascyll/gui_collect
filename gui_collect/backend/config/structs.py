...
class _GameConfigData:
    ...
    def __post_init__(self, _game):
        if not is_dataclass(self.game_options):
            ...
            self.multi_texture_select_options = _MultiTextureSelectOptions(**self.multi_texture_select_options)
        ...
        if self.multi_texture_select_options is None:
            self.multi_texture_select_options = _MultiTextureSelectOptions(
                only_selected_draw_call=True if _game in ['hsr', 'gi', 'hi3'] else False,
...
class ConfigData:
    ...
    def validate_config_data(d: dict):
        ...
        _validate_helper(d, default_config_data, [],       {'active_game', 'debugging', 'targeted_analysis_enabled', 'reverse_shapekeys_hsr', 'reverse_shapekeys_zzz', 'game'})
        _validate_helper(d, default_config_data, ['game'], {'zzz', 'hsr', 'gi', 'hi3'})
        for g in GAME_NAME:
            _validate_helper(d, default_config_data, ['game', g], {'extract_path', 'frame_analysis_parent_path', 'game_options', 'targeted_options', 'multi_texture_select_options'})
            ...
            _validate_helper(d, default_config_data, ['game', g, 'multi_texture_select_options'], {'only_selected_draw_call', 'ignore_texture_bleed', 'min_draw_call_id', 'max_draw_call_id'})

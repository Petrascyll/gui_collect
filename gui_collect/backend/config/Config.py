...
class Config:
    ...
    def set_config_key_value(
        self, key_path: list[str], value, is_path=False, echo=True
    ):
        d = self.data
        for i, key in enumerate(key_path):
            ...
            if type(d) is dict:
                ...
                d = d.__getattribute__(key)

        self.trigger_callbacks(key_path, value)

        if echo:
            if is_path:
                value = "<PATH>{}</PATH>".format(value)
    ...
    def _load_config(self) -> dict:
        with open(self._config_filepath, "r", encoding="utf-8") as f:
            try:
                ...
                print(
                    "\tconfig: JSON Decode Error. config.json is not a valid json file."
                )

...
def collect_binary_buffer_data(
        buffer_path: Path, buffer_formats: list[str], buffer_stride: int, *,
    ...
    if shapekey_buffer_path and shapekey_cb_paths:
        buffer = reverse_applied_shapekeys(bytearray(buffer), shapekey_buffer_path.read_bytes(), shapekey_cb_paths)
...
def reverse_applied_shapekeys(position_buffer, shapekey_buffer, shapekey_cb_paths: list[Path]):
    STRIDE = 40
    ...
    for cb_filepath in shapekey_cb_paths:
        ...
        logger.info(f"        <PATH>{cb_filepath.name}</PATH> [Offset: {offset:4}, Vertex Count: {vertex_count:4}, Multiplier: {multiplier}]")

...
def collect_binary_buffer_data(
    buffer_path: Path,
    buffer_formats: list[str],
    buffer_stride: int,
    *,
    shapekey_buffer_path: Path = None,
    shapekey_cb_paths: list[Path] = None,
):
    buffer = buffer_path.read_bytes()

    # May only be used for hsr/zzz extractions
    if shapekey_buffer_path and shapekey_cb_paths:
        buffer = reverse_applied_shapekeys(
            bytearray(buffer), shapekey_buffer_path.read_bytes(), shapekey_cb_paths
        )
...
def reverse_applied_shapekeys(
    position_buffer, shapekey_buffer, shapekey_cb_paths: list[Path]
):
    STRIDE = 40

    logger.info("Reversing Applied Shapkeys")

    prev_offset = 0
    prev_count = 0
    for cb_filepath in shapekey_cb_paths:
        offset, vertex_count, multiplier, garbage = struct.unpack_from(
            "<LLff", cb_filepath.read_bytes(), offset=0
        )

        if prev_offset + prev_count != offset:
            logger.warning("        Warning: missing shapekey metadata")
        logger.info(
            f"        <PATH>{cb_filepath.name}</PATH> [Offset: {offset:4}, Vertex Count: {vertex_count:4}, Multiplier: {multiplier}]"
        )
        ...
        for i in range(vertex_count):
            idx, *s = struct.unpack_from(
                "<L9f", shapekey_buffer, i * STRIDE + offset * STRIDE
            )

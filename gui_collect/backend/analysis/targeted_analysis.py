...
def generate(export_name, model_hashes, component_names, d3dx_path: Path, dump_rt = True, force_dump_dds = False, symlink = False, share_dupes = False):
    targeted_content = '\n\n'.join([
        '\n'.join([
    ...
    if d3dx_path:
        ...
        logger.info("Its relative path to <PATH>%s</PATH> is <PATH>%s</PATH>", d3dx_path.absolute(), relative_path, extra={'TIMESTAMP': False})

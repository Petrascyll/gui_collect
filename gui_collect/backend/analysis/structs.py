...
from gui_collect.backend.utils.texture_utils.texdiag_helper import get_texdiag_info
...
class Texture:
    ...
    def async_read_format(self, callback=None, *, blocking=False):
        ...
        if not self._read_format_thread:
            if callback:
                self._read_format_callbacks.append(callback)
    ...
    def async_read_width_height(self, callback=None, *, blocking=False):
        ...
        if self._width and self._height:
            if callback:
                return callback(self._width, self._height)
    ...
    def print(self, tabs=0):
        s = [
            ...
            *[
            ...
            "Draw VB0 Path: <PATH>{}</PATH>".format(
                self.backup_position_paths[0].with_suffix(".buf").name
                if self.backup_position_paths
                else ""
            ),
            "Drwa VB1 Path: <PATH>{}</PATH>".format(
                self.backup_texcoord_paths[0].with_suffix(".buf").name
                if self.backup_texcoord_paths
                else ""
            ),
            "Draw VB2 Path: <PATH>{}</PATH>".format(
                self.backup_draw_vb2_paths[0].with_suffix(".buf").name
                if self.backup_draw_vb2_paths
                else ""
            ),
            "",
            "Position Path: <PATH>{}</PATH>".format(
                self.position_path.with_suffix(".buf").name
                if self.position_path
                else ""
            ),
            "Texcoord Path: <PATH>{}</PATH>".format(
                self.texcoord_path.with_suffix(".buf").name
                if self.texcoord_path
                else ""
            ),
            "   Blend Path: <PATH>{}</PATH>".format(
                self.blend_path.with_suffix(".buf").name if self.blend_path else ""
            ),
        ]
        ...
        for line in s:
            logger.info(line, extra={"TIMESTAMP": False})

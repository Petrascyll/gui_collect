...
class LogAnalysis():
    ...
    def extract(self, extract_component: Component, component_hash: str, component_hash_type: str = None, game = 'zzz', reverse_shapekeys=False) -> None:
        ...
        if not component_hash_type:
            ...
            if not component_hash_type:
                logger.error("ERROR: Failed to find hash \"%s\" in FrameAnalysis log file.", component_hash)
                raise ZeroDivisionError()
            if component_hash_type not in [BufferType.IB, BufferType.Draw_VB]:
                logger.error("ERROR: Hash \"%s\" is neither an IB nor a Draw hash.", component_hash)
                raise ZeroDivisionError()
        ...
        if str(self.frame_analysis_path.absolute()).isascii():
            ...
            logger.warning("Non ASCII characters detected in frame analysis path. Falling back to texdiag.exe to find texture formats.")
        ...
        for id in component.ids:
            ...
            if ib_first_index not in object_indices:
                ...
                if not ib_filepath.exists():
                    logger.error("ERROR: <PATH>%s</PATH> does not exist in the frame analysis folder.", ib_filepath.name)
                    logger.warning("Your `analyse_options` [Hunting] 3dm key may be missing required values.", extra={'TIMESTAMP': False})
                    raise BufferError()
        ...
        for shapekey_id in shapekey_ids:
            ...
            for slot, cb_hash in self.log_data[shapekey_id]['CSSetConstantBuffers'].items():
                ...
                if not cb_filepath_buf.exists():
                    logger.warning("<PATH>%s</PATH> does not exist. Discovered shapekey data cannot be extracted!", cb_filepath_buf.name)
                    ...
                    if deduped_textures[tex.slot].hash != tex.hash:
                        logger.warning(
                            "Skipping texture with hash %s at slot %s colliding with texture %s at the same slot.",
                                tex.hash, tex.slot, deduped_textures[tex.slot].hash
                        )
                    # Override existing if the texture we're proccessing now is .dds
                        ...
                        logger.warning(
                            "Discarded %s variant of texture %s in favor of dds variant.",
                                deduped_textures[tex.slot].extension, tex.hash
                        )
                        deduped_textures[tex.slot] = tex
    ...
    def check_analysis(self, component: Component):
        ...
        for k, txt_path in txt_paths.items():
            ...
            if missing_path:
                logger.error("%s: <PATH>%s</PATH> does not exist in the frame analysis folder.", k, missing_path.name)
                logger.warning("Make sure to include `buf` in addition to `txt` in your `analyse_options` [Hunting] 3dm key.", extra={"TIMESTAMP": False})
                logger.warning("If you're performing targeted dumps, also check your targeted .ini is correctly targeting the model.", extra={"TIMESTAMP": False})
                raise BufferError()
    ...
    with open(log_path, 'r', encoding=encoding) as log_file:
        ...
        while line := log_file.readline():
            if m := re.match(r'\d{6}', line):
                ...
                continue
    logger.info("Read {} in {:.3}s".format(log_path.parent.name + '\\' + log_path.name, time.time() - st))

...
from gui_collect.backend.utils.buffer_utils.structs import BufferType
...
class LogAnalysis:
    def __init__(self, frame_analysis_path: Path):
        ...
        self.log_data = parse_frame_analysis_log_file(
            self.frame_analysis_path / "log.txt"
        )
    ...
    def extract(
        ...
        component_hash: str,
        component_hash_type: str = None,
        game="zzz",
        reverse_shapekeys=False,
    ) -> None:
        st = time.time()
        if not component_hash_type:
            ...
            if not component_hash_type:
                logger.error(
                    'ERROR: Failed to find hash "%s" in FrameAnalysis log file.',
                    component_hash,
                )
        ...
        if str(self.frame_analysis_path.absolute()).isascii():
            ...
            logger.warning(
                "Non ASCII characters detected in frame analysis path. Falling back to texdiag.exe to find texture formats."
            )
            self.set_textures_from_dir(extract_component)

        self.check_analysis(extract_component)
        logger.info("Log Based Extraction Done: {:.6}s".format(time.time() - st))

    def guess_hash_type(self, target_hash):
        for id in self.log_data:
            if (
                "IASetIndexBuffer" in self.log_data[id]
    ...
    def set_draw_data(self, component: Component):
        ...
        for id in component.ids:
            ...
            if ib_first_index not in object_indices:
                ...
                if not ib_filepath.exists():
                    logger.error(
                        "ERROR: <PATH>%s</PATH> does not exist in the frame analysis folder.",
                        ib_filepath.name,
                    )
                    logger.warning(
                        "Your `analyse_options` [Hunting] 3dm key may be missing required values.",
        ...
        component.ib_paths = [
            ib_filepath
            for ib_filepath, _ in sorted(
                zip(ib_filepaths, object_indices), key=lambda item: item[1]
            )
        ]
    ...
    def get_cs_pose_id(self, component: Component):
        ...
        root_cs_ids = sorted([
            ...
            and component.draw_hash
            in self.log_data[id]["CSSetUnorderedAccessViews"].values()
        ])
        ...
        pose_ids = [
            id
            for id in root_cs_ids
            if "CSSetShaderResources" in self.log_data[id]
            and component.draw_vb2_hash
            in self.log_data[id]["CSSetShaderResources"].values()
        ]
        if len(pose_ids) == 0:
            return None
    ...
    def set_cs_prepose_data(self, component: Component, root_cs_id: str, pose_id: str):
        ...
        if "CopyResource" in self.log_data[root_cs_id] and (
            src_uav_hashes := [
                src_hash
                for src_hash, dst_hash in self.log_data[root_cs_id]["CopyResource"]
                if dst_hash == component.draw_hash
            ]
        ):
            position_slot = "0"
            position_hash = src_uav_hashes[0]
            position_path = self.compile_cs_u_filepath(
                root_cs_id, component.draw_hash, position_slot, root_cs_hash, ext=".buf"
            )

            cst0_hash = self.log_data[root_cs_id]["CSSetShaderResources"]["0"]
            cst0_filename = self.compile_cs_t_filepath(
                root_cs_id, cst0_hash, 0, root_cs_hash, ".buf"
            )

            component.shapekey_buffer_path = self.frame_analysis_path / cst0_filename
            for slot, cb_hash in self.log_data[root_cs_id][
                "CSSetConstantBuffers"
            ].items():
                cb_filename_buf = self.compile_cs_cb_filepath(
                    root_cs_id, cb_hash, slot, root_cs_hash, ".buf"
                )
            ...
            position_slot = [
                slot
                for slot in self.log_data[root_cs_id][
                    "CSSetUnorderedAccessViews"
                ].keys()
                if self.log_data[root_cs_id]["CSSetUnorderedAccessViews"][slot]
                == component.draw_hash
            ][0]
            position_hash = self.log_data[root_cs_id]["CSSetShaderResources"][
                position_slot
            ]
            position_path = self.compile_cs_t_filepath(
                root_cs_id, position_hash, position_slot, root_cs_hash, ext=".buf"
            )

        blend_slot = [
            slot
            for slot in self.log_data[pose_id]["CSSetShaderResources"].keys()
            if self.log_data[pose_id]["CSSetShaderResources"][slot]
            == component.draw_vb2_hash
        ][0]
        blend_hash = self.log_data[pose_id]["CSSetShaderResources"][blend_slot]
        blend_path = self.compile_cs_t_filepath(
            pose_id, blend_hash, blend_slot, pose_cs_hash, ext=".buf"
        )
        ...
        if len(pose_ids) > 1:
            ...
            if prepose_buffer_count == 3:
                pose_ids = [
                    id
                    for id in pose_ids
                    if self.log_data[id]["IASetVertexBuffers"]["1"]
                    == component.texcoord_hash
                ]
    ...
    def set_prepose_data(self, component: Component, pose_id: str):
        ...
        if prepose_buffer_count == 2:
            prepose_data = (["0", "1"], [BufferType.Position_VB, BufferType.Blend_VB])
        elif prepose_buffer_count == 3:
            prepose_data = (
                ["0", "1", "2"],
                [BufferType.Position_VB, BufferType.Texcoord_VB, BufferType.Blend_VB],
            )
        else:
            raise Exception()
        ...
        for slot, buffer_type in zip(*prepose_data):
            ...
            buffer_path = self.compile_vb_filepath(
                pose_id, buffer_hash, slot, vs_hash, ext=".txt"
            )
    ...
    def set_shapekey_data(self, component: Component, pose_id: str):
        ...
        cst0_filename = self.compile_cs_t_filepath(
            shapekey_ids[0], cst0_hash, 0, cs_hash, ".buf"
        )
        ...
        for shapekey_id in shapekey_ids:
            ...
            for slot, cb_hash in self.log_data[shapekey_id][
                "CSSetConstantBuffers"
            ].items():
                cb_filename_buf = self.compile_cs_cb_filepath(
                    shapekey_id, cb_hash, slot, cs_hash, ".buf"
                )
                cb_filepath_buf = self.frame_analysis_path / cb_filename_buf
                ...
                if not cb_filepath_buf.exists():
                    logger.warning(
                        "<PATH>%s</PATH> does not exist. Discovered shapekey data cannot be extracted!",
                        cb_filepath_buf.name,
                    )
        ...
        for first_index in component.draw_data:
            for draw_call_id in component.draw_data[first_index]:
                ...
                if game != "gi" and "render_targets" in self.log_data[draw_call_id]:
                    has_o0 = any([
                        rt["slot"] == "0"
                        for rt in self.log_data[draw_call_id]["render_targets"].values()
                    ])
                    if has_o0:
                        component.tex_index_id[first_index] = draw_call_id
                        break
            ...
            # Default to first draw id if no good draw id can be found
                ...
                component.tex_index_id[first_index] = next(
                    iter(component.draw_data[first_index])
                )
            ...
            for id in component.draw_data[first_index]:
                ...
                component.draw_data[first_index][id].textures = sorted(
                    [
                        ...
                        for texture_str_filepath, texture_info in self.log_data[id][
                            "textures"
                        ].items()
                ...
                for tex in component.draw_data[first_index][id].textures:
                    ...
                    if deduped_textures[tex.slot].hash != tex.hash:
                        logger.warning(
                            "Skipping texture with hash %s at slot %s colliding with texture %s at the same slot.",
                            tex.hash,
                            tex.slot,
                            deduped_textures[tex.slot].hash,
                        )
                    # Override existing if the texture we're proccessing now is .dds
                    elif tex.extension == "dds":
                        logger.warning(
                            "Discarded %s variant of texture %s in favor of dds variant.",
                            deduped_textures[tex.slot].extension,
                            tex.hash,
                        )
                        deduped_textures[tex.slot] = tex

                component.draw_data[first_index][id].textures = list(
                    deduped_textures.values()
                )
                ...
                if id == initial_id:
                    for texture in component.draw_data[first_index][id].textures:
                        self.texture_manager.get_image(
                            texture, 256, callback=lambda a, b, c: None
                        )
    ...
    def check_analysis(self, component: Component):
        ...
        txt_paths: dict[str, Path] = {
            ...
            "Backup Position Data": component.backup_position_paths[0]
            if component.backup_position_paths
            else None,
            "Backup Texcoord Data": component.backup_texcoord_paths[0]
            if component.backup_texcoord_paths
            else None,
        }
        for k, txt_path in txt_paths.items():
            ...
            if not buf_path.exists():
                missing_path = buf_path
            if missing_path:
                logger.error(
                    "%s: <PATH>%s</PATH> does not exist in the frame analysis folder.",
                    k,
                    missing_path.name,
                )
                logger.warning(
                    "Make sure to include `buf` in addition to `txt` in your `analyse_options` [Hunting] 3dm key.",
                    extra={"TIMESTAMP": False},
                )
                logger.warning(
                    "If you're performing targeted dumps, also check your targeted .ini is correctly targeting the model.",
                    extra={"TIMESTAMP": False},
                )
                raise BufferError()
    ...
    def compile_vb_filepath(
        self, draw_id, vb_hash, slot, vs_hash, ps_hash=None, ext=".txt"
    ):
        resources = [(f"vb{slot}", vb_hash), ("vs", vs_hash)]
        if ps_hash:
            resources.append(("ps", ps_hash))
    ...
    def compile_filepath(self, draw_id, resources: list[tuple[str, str]], ext=".txt"):
        filename = "{}-{}{}".format(
            draw_id,
            "-".join([
                f"{resource}={resource_hash}" for resource, resource_hash in resources
            ]),
            ext,
        )
    ...
    def get_ib_index_count(self, draw_id: str) -> int:
        if "DrawIndexedInstanced" in self.log_data[draw_id]:
            return int(
                self.log_data[draw_id]["DrawIndexedInstanced"]["IndexCountPerInstance"]
            )
        elif "DrawIndexed" in self.log_data[draw_id]:
            return int(self.log_data[draw_id]["DrawIndexed"]["IndexCount"])
        else:
    ...
    def get_ib_first_index(self, draw_id: str) -> int:
        if "DrawIndexedInstanced" in self.log_data[draw_id]:
            return int(
                self.log_data[draw_id]["DrawIndexedInstanced"]["StartIndexLocation"]
            )
        elif "DrawIndexed" in self.log_data[draw_id]:
            return int(self.log_data[draw_id]["DrawIndexed"]["StartIndexLocation"])
        else:
            print(self.log_data[draw_id])
            raise Exception()
...
def parse_frame_analysis_log_file(log_path: Path):
    ...
    for encoding in ["UTF-8", "cp1252"]:
        ...
        logger.error(
            "Malformed log.txt. Failed to decode log file using UTF-8 or cp1252 encodings"
        )
        return None
            ...
            if m := re.match(r"\d{6}", line):
                    ...
                    if int(draw_id) > 1:
                        ...
                        try:
                            if (
                                ...
                                log_data[prev_draw_id]["IASetIndexBuffer"] = log_data[
                                    prev_prev_draw_id
                                ]["IASetIndexBuffer"]
                        except:
                ...
                if line[7:15] == "3DMigoto":
                    if m := LOG_TEXTURE_PATTERN.match(line):
                        filepath = Path(
                            log_path.parent, m.group("texture_filename")
                        ).absolute()
                        if m.group("resource_prefix") == "ps-t":
                            ...
                            log_data[draw_id]["textures"][str(filepath)] = {
                                ...
                                "contamination": m.group("contamination")[:-1]
                                if m.group("contamination")
                                else "",
                                "extension": m.group("extension"),
                                "bleed": (
                                    True
                                    if "PSSetShaderResources" not in log_data[draw_id]
                                    else True
                                    if m.group("texture_slot")
                                    not in log_data[draw_id]["PSSetShaderResources"]
                                    else m.group("texture_hash")
                                    not in log_data[draw_id]["PSSetShaderResources"][
                                        m.group("texture_slot")
                                    ]
                                ),
                            }
                        else:
                ...
                if keyword in [
                    ...
                    while s := re.match(
                        r"^\s*(.*):(?: view=0x.*?)? resource=.*? hash=(.*)$", line
                    ):
                    ...
                    assert keyword not in log_data[draw_id]
                    log_data[draw_id][keyword] = {arg[0]: arg[1] for arg in args}

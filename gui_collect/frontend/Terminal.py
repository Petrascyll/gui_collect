...
class Terminal(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.chevron_down_img = tk.PhotoImage(
            file=Path("resources", "images", "buttons", "chevron.down.16.png")
        )
        self.chevron_up_img = tk.PhotoImage(
            file=Path("resources", "images", "buttons", "chevron.up.16.png")
        )
    ...
    def create_widgets(self):
        ...
        self.text_wdgt.tag_config(
            "LINK", foreground="#08F", font=("Lucida Sans Typewriter", 10, "underline")
        )
        self.text_wdgt.tag_config(
            "ERROR", foreground="#F44", font=("Lucida Sans Typewriter", 10, "bold")
        )
        self.text_wdgt.tag_config(
            "WARNING", foreground="#FB2", font=("Lucida Sans Typewriter", 10, "bold")
        )

        self.btn = FlatImageButton(
            self,
            image=self.chevron_down_img,
            bg=self["bg"],
            img_width=16,
            img_height=16,
            width=34,
    ...
    def toggle_terminal_visible(self, event):
        ...
        if self.terminal_visible:
            self.btn.set_image(
                self.chevron_down_img, img_width=16, img_height=16, width=32, height=32
            )
            self.master.sash_place(0, 0, int(root_height * 0.75))

        else:
            self.btn.set_image(
                self.chevron_up_img, img_width=16, img_height=16, width=32, height=32
            )

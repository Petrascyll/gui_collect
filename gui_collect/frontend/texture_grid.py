...
class TextureGrid(tk.Frame):
    ...
    def create_header(self):
        ...
        self.multi_mode_indicator = tk.Label(
            self.header,
            text="M",
            font=("Arial", 16, "bold"),
            width=3,
        ...
        self.multi_mode_indicator.grid(
            row=0, rowspan=2, column=0, padx=(0, 8), sticky="nsew"
        )
        ...
        m.add_command(
            label="Copy Vertex Shader Hash", command=partial(handle_copy, self.vs_label)
        )
        m.add_command(
            label="Copy Pixel Shader Hash", command=partial(handle_copy, self.ps_label)
        )
        ...
        self.ext_filter = TextFilter(
            self.header,
            callback=self.regrid_active_widgets,
            active=False,
            values=("", "dds", "jpg"),
            value_labels=("", ".dds", ".jpg"),
            initial_value_index=0,
        )
        self.width_filter = ConditionalFilter(
            self.header,
            callback=self.regrid_active_widgets,
            active=True,
            label_text="W",
        ...
        self.height_filter = ConditionalFilter(
            self.header,
            callback=self.regrid_active_widgets,
            active=True,
            label_text="H",
        ...
        self.size_filter = ConditionalFilter(
            self.header,
            callback=self.regrid_active_widgets,
            active=False,
            label_text="S",
            values=size_values,
            value_labels=size_value_labels,
            initial_value_index=2,
        )
        self.pow_2_filter = FlatImageButton(
            self.header,
            image=tk.PhotoImage(
                file=Path("resources", "images", "buttons", "pow_2.inverted.44.png")
            ),
            dual_state=True,
            key="pow_2",
            value=True,
        ...
        self.pow_2_filter.bind(
            "<Button-1>", add="+", func=lambda _: self.regrid_active_widgets()
        )
    ...
    def load(self, components: list[Component], component_index: int, first_index: int):
        ...
        self.vs_label.config(
            text=f"VS Hash: {self.components[component_index].draw_data[first_index][self.active_id].vs_hash}"
        )
        self.ps_label.config(
            text=f"PS Hash: {self.components[component_index].draw_data[first_index][self.active_id].ps_hash}"
        )
        ...
        for i, component in enumerate(components):
            ...
            for first_index in component.object_indices:
                ...
                self.frames[i][first_index] = {
                    id: None for id in component.draw_data[first_index]
                }
                self.frames[i][first_index][initial_id] = frame
                self.frames[i][first_index]["active_id"] = initial_id

        self.frames[self.component_index][self.first_index][self.active_id].bind(
    ...
    def regrid_active_widgets(self):
        active_frame = self.frames[self.component_index][self.first_index][
            self.active_id
        ]
        textures = (
            self
            .components[self.component_index]
            .draw_data[self.first_index][self.active_id]
            .textures
        )
    ...
    def refresh_id_picker(self, component_index: int, first_index: int):
        ...
        for id in self.frames[component_index][first_index]:
            ...
            if id == active_id:
                ...
                id_label.bind(
                    "<Button-1>",
                    partial(self.set_active_grid_id, component_index, first_index, id),
                )
            id_label.pack(side="top", pady="2", ipady="4", ipadx="8", anchor="center")

    def set_active_grid_id(
        self, component_index: int, first_index: int, id: str, event
    ):
    ...
    def set_active_grid(self, component_index, first_index):
        ...
        self.frames[self.component_index][self.first_index][self.active_id].unbind_all(
            "<Configure>"
        )
        ...
        self.vs_label.config(
            text=f"VS Hash: {self.components[self.component_index].draw_data[self.first_index][self.active_id].vs_hash}"
        )
        self.ps_label.config(
            text=f"PS Hash: {self.components[self.component_index].draw_data[self.first_index][self.active_id].ps_hash}"
        )

        self.regrid_active_widgets()
        self.frames[self.component_index][self.first_index][
            self.active_id
    ...
    def grid_widget(self, i, item):
        ...
        if self.padding_style == "even":
            ...
            # ex: [------X--X--X------]
            padding = (
                self.empty_space - ((self.column_count - 1) * self.min_padx)
            ) // 2
            if column == first:
                padx = (padding, self.min_padx // 2)
            elif column == last:
                padx = (self.min_padx // 2, padding)
            else:
                padx = (self.min_padx // 2, self.min_padx // 2)
            if row == first:
                pady = 0
            else:
                pady = self.pady

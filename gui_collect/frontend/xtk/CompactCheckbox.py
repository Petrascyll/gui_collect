...
from gui_collect.frontend.style import brighter, darker
...
class CompactCheckbox(tk.Canvas):
    def __init__(
        ...
        **kwargs,
        ...
        if not self.fill:
            text_width = Font(font=self.font).measure(text)
    ...
    def draw(self, total_width):
        ...
        self.text_id = self.create_text(
            text_x, h // 2, anchor="w", text=self.text, font=self.font, fill="#CCC"
        )
        self.border_id = self.create_rectangle(
            border_x0, 0, border_x1, h, outline="", fill=self.active_bg
        )
        self.inner_bg = self.create_rectangle(
            inner_bg_x0, bt, inner_bg_x1, h - bt, outline="", fill=self["bg"]
        )
        self.core_id = self.create_rectangle(
            core_id_x0, bt + g, core_id_x1, h - bt - g, outline="", fill=self.active_bg
        )

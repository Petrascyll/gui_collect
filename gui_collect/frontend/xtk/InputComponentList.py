...
class InputComponentFrameList(tk.Frame):
    ...
    def get(self) -> list[InputComponent]:
        return [
            input_component_frame.get()
            for input_component_frame in self.input_component_frames
        ]
    ...
    def add_input_component_frame(self):
        ...
        self.input_component_frames.append(
            InputComponentFrame(
                self, index=i, handle_remove=self.remove_input_component_frame
            )
        )
        self.input_component_frames[-1].pack(
            side="top", padx=16, anchor="w", fill="x", pady=(0, 8)
        )
        ...
        self.input_component_frames[-1].component_hash_entry.bind(
            "<KeyRelease>", lambda e: self.hash_key_release(e, i)
        )
...
class InputComponentFrame(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.component_options_frame.grid(
            row=0, column=1, rowspan=2, sticky="nsew", padx=(2, 0)
        )
        ...
        boolean_options = [
            (
                ...
                True,
                tk.PhotoImage(
                    file=Path("resources", "images", "buttons", "cube.inverted.74.png")
                ),
                "Collect model data. Model data will be extracted to the output folder.",
            ),
            (
                "collect_model_hashes",
                True,
                tk.PhotoImage(
                    file=Path("resources", "images", "buttons", "hash.inverted.74.png")
                ),
                "Collect model hashes. Model hashes will be written to the hash.json in the output folder.",
            ),
            (
                "collect_texture_data",
                True,
                tk.PhotoImage(
                    file=Path("resources", "images", "buttons", "dds.inverted.74.png")
                ),
                "Collect texture data. Texture data will be copied to the output folder.",
            ),
            (
                "collect_texture_hashes",
                True,
                tk.PhotoImage(
                    file=Path("resources", "images", "buttons", "hash.inverted.74.png")
                ),
                "Collect texture hashes. Texture hashes will be written to the hash.json in the output folder.",
            ),
        ]

        for i, (option_key, initial_value, img, option_text) in enumerate(
            boolean_options
        ):

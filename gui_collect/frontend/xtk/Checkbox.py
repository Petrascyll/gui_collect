...
class Checkbox(tk.Canvas):
    ...
    def __init__(self, parent, active=False, disabled=False, *args, **kwargs):
        ...
        self.border = tk.PhotoImage(
            file=Path("./resources/images/buttons/checkbutton_border.32.png")
        )
        self.core = tk.PhotoImage(
            file=Path("./resources/images/buttons/checkbutton_core.18.32.png")
        )

        self.border_id = self.create_image(0, 0, anchor="nw", image=self.border)
        self.core_id = self.create_image(
            0, 0, anchor="nw", image=self.core, state="hidden"
        )
...
class LabeledCheckbox(tk.Frame):
    def __init__(
        self, parent, text, font, disabled=False, initial_state=False, *args, **kwargs
    ):

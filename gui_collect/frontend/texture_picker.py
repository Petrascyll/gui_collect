...
from gui_collect.backend.analysis.structs import Component, Texture
...
class TexturePicker(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.texture_grid = TextureGrid(
            self, get_ref=self.texture_bar.get_component_part_frame
        )
    ...
    def load(
        ...
        texture_components_idx: list[int],
        finish_extraction_callback,
    ):
        logger.info("Texture picker loaded")
        ...
        def helper_callback(collected_textures):
            ...
            components_textures = [
                ...
                if i not in texture_components_idx
                else next(iter_collected_textures)
        ...
        self.texture_grid.load(
            component_index=0, first_index=0, components=texture_components
        )
        self.texture_bar.load(
            component_index=0, first_index=0, components=texture_components
        )
        self.bind_keys()
        self.bind_modifiers()
...
class TextureBar(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.cancel.grid(
            sticky="nsew", row=1, column=0, padx=(0, 2), ipadx=56, ipady=16
        )
    ...
    def populate_widgets(self):
        ...
        for i, component in enumerate(self.components):
            for j, first_index in enumerate(sorted(component.object_indices)):
                ...
                component_part_name = "{}{}".format(
                    component.name, component.object_classification[j]
                )
                component_part = ComponentPartFrame(
                    self.component_summary.interior,
                    active=is_active,
                    header_text=component_part_name,
                )
                component_part.header.bind(
                    "<Button-1>", partial(self.handle_jump, i, first_index)
                )
    ...
    def load(self, component_index, first_index, components: list[Component]):
        ...
        self.component_sequence = [
            component.object_indices for component in self.components
        ]
...
class ComponentPartFrame(tk.Frame):
    ...
    def add_texture(self, texture: Texture, texture_type: str):
        c = ComponentPartTextureFrame(
            self, texture, texture_type, handle_remove=self.handle_remove
        )
    ...
    def get_textures(self):
        ...
        return [item[1].get_texture() for item in sorted_textures]
...
class ComponentPartTextureFrame(tk.Frame):
    def __init__(
        ...
        **kwargs,
        ...
        TextureManager.get_instance().get_image(
            self.texture, max_width=256, callback=self.set_thumbnail_image
        )
    ...
    def create_widgets(self):
        ...
        self.thumbnail_canvas = tk.Canvas(
            self,
            width=256 // self.sub_level,
            height=256 // self.sub_level,
            bg="#111",
            highlightthickness=0,
        )
        self.thumbnail_canvas.grid(
            row=0, column=1, padx=(0, 2), rowspan=2, sticky="nsew"
        )

        texture_type_label = tk.Label(
            self,
            text=self.texture_type,
            font=("Arial", 20, "bold"),
            bg="#333",
            fg="#e8eaed",
        )
        ...
        img = tk.PhotoImage(
            file=Path("./resources/images/buttons/close.256.png").absolute()
        ).subsample(8)
        remove_btn = FlatImageButton(
            button_frame,
            width=32,
            height=32,
            img_width=32,

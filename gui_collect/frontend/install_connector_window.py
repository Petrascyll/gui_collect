...
def __iterate_ini_dir(root_path: Path):
    for p in root_path.iterdir():
        if p.name.lower().startswith("disabled"):
            continue
...
def open_install_connector_window(root):
    ...
    def handle_path_change(mods_path: Path):
        ...
        text_wdgt.insert(
            tk.END,
            "Checking if valid connector is already installed in mods directory...\n",
            "GREEN",
        )
        ...
        for p in __iterate_ini_dir(mods_path):
            ...
            if include_pattern.search(text):
                text_wdgt.insert(
                    tk.END,
                    "\nValid gui_collect connector .ini already installed ".format(
                        str(p.absolute())
                    ),
                    "WARNING",
                )
                ...
                text_wdgt.insert(
                    tk.END, "No additional actions required.\n\n", "WARNING"
                )
                text_wdgt.see(tk.END)
            ...
            if target_path.exists():
                ...
                text_wdgt.insert(
                    tk.END, "Installation will OVERWRITE it!\n\n", "WARNING"
                )
                text_wdgt.see(tk.END)
    ...
    pp_widget = __create_path_picker_widget(
        top_window, top_frame, callback=handle_path_change, text_wdgt=text_wdgt
    )
...
def __create_text_widget(top_window, top_frame):
    ...
    text_wdgt.tag_config(
        "WARNING", foreground="#FB2", font=("Lucida Sans Typewriter", 10, "bold")
    )
    text_wdgt.tag_config(
        "GREEN", foreground="#6F6", font=("Lucida Sans Typewriter", 10, "bold")
    )
...
def __create_path_picker_widget(top_window, top_frame, callback, text_wdgt):
    ...
    def handle_click(pp: PathPicker, new_path: str):
        ...
        if (path / "d3dx.ini").exists() and (path / "Mods").exists():
            text_wdgt.insert(
                tk.END, f"Found valid 3dm mods folder path: {str(path)}\n\n", "GREEN"
            )
            path = path / "Mods"
            ...
            text_wdgt.insert(
                tk.END, f"Got valid 3dm mods folder path: {str(path)}\n\n", "GREEN"
            )
            pp.default_bg = "#0A0"
            pp.refresh_label_bindings()
    ...
    lbl = tk.Label(
        frame,
        text="Select 3dmigoto Mods Folder: ",
        font=("Arial", 16),
        fg="#CCC",
        bg="#333",
        relief="flat",

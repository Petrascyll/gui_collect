...
class ExtractForm(tk.Frame):
    ...
    def create_targeted_options(self):
        ...
        def handle_change_0(newValue: bool):
            logger.info('Set Config: /game/%s/targeted_options/force_dump_dds = %s', self.variant.value, newValue)
            targeted_options.force_dump_dds = newValue
        def handle_change_1(newValue: bool):
            logger.info('Set Config: /game/%s/targeted_options/dump_rt = %s', self.variant.value, newValue)
            targeted_options.dump_rt = newValue
        def handle_change_2(newValue: bool):
            logger.info('Set Config: /game/%s/targeted_options/symlink = %s', self.variant.value, newValue)
            targeted_options.symlink = newValue
        def handle_change_3(newValue: bool):
            logger.info('Set Config: /game/%s/targeted_options/share_dupes = %s', self.variant.value, newValue)
    ...
    def create_extract_options(self):
        ...
        def handle_change_0(newValue: bool):
            logger.info('Set Config: /game/%s/clean_extract_folder = %s', self.variant.value, newValue)
            game_options.clean_extract_folder  = newValue
        def handle_change_1(newValue: bool):
            logger.info('Set Config: /game/%s/open_extract_folder = %s', self.variant.value, newValue)
            game_options.open_extract_folder   = newValue
        def handle_change_2(newValue: bool):
            logger.info('Set Config: /game/%s/delete_frame_analysis = %s', self.variant.value, newValue)
    ...
    def start_extraction(self):
        ...
        if not input_component_hashes:
            ...
            return
        if not extract_name:
            logger.error("Frame Analysis Aborted! You must provided a name for the extracted model.")
            return
        if not (path/'log.txt').exists():
            logger.error("Frame Analysis Aborted! Invalid frame analysis path: \"%s\".", str(path))
            return

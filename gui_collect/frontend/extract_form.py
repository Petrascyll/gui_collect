...
from gui_collect.backend.analysis.FrameAnalysis import FrameAnalysis
from gui_collect.backend.utils import is_valid_hash
...
class ExtractForm(tk.Frame):
    ...
    def create_widgets(self):
        ...
        extract_button = FlatButton(
            self.extract_frame, text="Extract", bg="#A00", hover_bg="#F00"
        )
    ...
    def create_component_options(self):
        self.input_component_list = InputComponentFrameList(
            self.component_options_frame.interior
        )
        self.input_component_list.pack(
            anchor="w", padx=(0, 4), pady=(16, 0), fill="both"
        )

    def create_targeted_options(self):
        targeted_dump_frame_title = tk.Label(
            self.targeted_dump,
            text="Targeted Frame Analysis",
            bg="#222",
            fg="#555",
            anchor="w",
            font=("Arial", "16", "bold"),
        ...
        pp = PathPicker(
            self.targeted_dump,
            value=Path("include", "auto_generated.ini"),
            callback=None,
            bg="#333",
            button_bg=self.accent_color,
        )
        ...
        def handle_change_0(newValue: bool):
            logger.info(
                "Set Config: /game/%s/targeted_options/force_dump_dds = %s",
                self.variant.value,
                newValue,
            )
        ...
        def handle_change_1(newValue: bool):
            logger.info(
                "Set Config: /game/%s/targeted_options/dump_rt = %s",
                self.variant.value,
                newValue,
            )
        ...
        def handle_change_2(newValue: bool):
            logger.info(
                "Set Config: /game/%s/targeted_options/symlink = %s",
                self.variant.value,
                newValue,
            )
        ...
        def handle_change_3(newValue: bool):
            logger.info(
                "Set Config: /game/%s/targeted_options/share_dupes = %s",
                self.variant.value,
                newValue,
            )
        ...
        checkbox_frame = tk.Frame(
            self.targeted_dump, height=100, bg=self.targeted_dump["bg"]
        )
        ...
        checkbox_0 = CompactCheckbox(
            ...
            on_change=handle_change_0,
            active=targeted_options.force_dump_dds,
            text="Force dump textures as .dds",
        ...
        checkbox_1 = CompactCheckbox(
            ...
            bg="#222",
            active_bg=self.accent_color,
            on_change=handle_change_1,
            active=targeted_options.dump_rt,
            text="Dump render targets",
            tooltip_text=(
        ...
        checkbox_2 = CompactCheckbox(
            ...
            bg="#222",
            active_bg=self.accent_color,
            on_change=handle_change_2,
            active=targeted_options.symlink,
            text="Symlink",
            tooltip_text=(
        ...
        checkbox_3 = CompactCheckbox(
            ...
            bg="#222",
            active_bg=self.accent_color,
            on_change=handle_change_3,
            active=targeted_options.share_dupes,
            text="Share dupes",
        ...
        generate_targeted_button = FlatButton(
            self.targeted_dump,
            text="Generate",
            bg=self.accent_color,
            hover_bg=brighter(self.accent_color),
        )
        generate_targeted_button.bind(
            "<Button-1>", lambda _: self.generated_targeted_dump_ini()
        )
        generate_targeted_button.pack(side="right", ipadx=10, ipady=10, padx=(8, 0))

        clear_targeted_button = FlatButton(
            self.targeted_dump,
            text="Clear",
            bg=self.accent_color,
            hover_bg=brighter(self.accent_color),
        )
        clear_targeted_button.bind(
            "<Button-1>", lambda _: self.clear_targeted_dump_ini()
        )
    ...
    def create_extract_options(self):
        ...
        def handle_path_change(newPath: str):
            ...
            logger.info(
                "Set Config: /game/extract/%s/extract_path = <PATH>%s</PATH>",
                self.variant.value,
                newPath,
            )
        ...
        pp = PathPicker(
            self.extract_options_frame,
            value=self.cfg.data.game[self.variant.value].extract_path,
            callback=handle_path_change,
            bg="#333",
            button_bg=self.accent_color,
            editable=True,
        )
        ...
        def handle_change_0(newValue: bool):
            logger.info(
                "Set Config: /game/%s/clean_extract_folder = %s",
                self.variant.value,
                newValue,
            )
        ...
        def handle_change_1(newValue: bool):
            logger.info(
                "Set Config: /game/%s/open_extract_folder = %s",
                self.variant.value,
                newValue,
            )
        ...
        def handle_change_2(newValue: bool):
            logger.info(
                "Set Config: /game/%s/delete_frame_analysis = %s",
                self.variant.value,
                newValue,
            )
        ...
        checkbox_0 = CompactCheckbox(
            ...
            height=30,
            active_bg=self.accent_color,
            active=game_options.clean_extract_folder,
            on_change=handle_change_0,
            text="Clean extracted folder before extraction",
        )
        checkbox_1 = CompactCheckbox(
            self.extract_options_frame,
            height=30,
            active_bg=self.accent_color,
            active=game_options.open_extract_folder,
            on_change=handle_change_1,
            text="Open extracted folder after extraction",
        )
        checkbox_2 = CompactCheckbox(
            self.extract_options_frame,
            height=30,
            active_bg=self.accent_color,
            active=game_options.delete_frame_analysis,
            on_change=handle_change_2,
            text="Delete frame analysis after extraction",
        )
    ...
    def grid_widgets(self):
        self.component_options_frame.grid(
            column=0, row=0, padx=(16, 0), pady=16, sticky="nsew", rowspan=3
        )
        self.extract_options_frame.grid(
            column=1, row=0, padx=16, pady=(16, 0), sticky="nsew"
        )
        if self.cfg.data.targeted_analysis_enabled:
            self.targeted_dump.grid(
                column=1, row=1, padx=16, pady=(16, 0), sticky="nsew"
            )
            self.extract_frame.grid(column=1, row=2, padx=16, pady=16, sticky="nsew")
        else:
            self.extract_frame.grid(
                column=1, row=1, rowspan=2, padx=16, pady=16, sticky="nsew"
            )
    ...
    def collect_input(self, skip_collect_nothing=True):
        ...
        for input_component in self.input_component_list.get():
            if not input_component.hash:
                continue
            if not is_valid_hash(input_component.hash):
                logger.warning("<ERROR>Invalid hash: %s</ERROR>", input_component.hash)
                return None, None, None, None, None

            # Don't include components with none of the collect options enabled
            if skip_collect_nothing:
                collect_nothing = all(
                    option_value is False
                    for option_value in input_component.options.values()
                )
                if collect_nothing:
                    continue
    ...
    def generated_targeted_dump_ini(self):
        extract_name, input_component_hashes, input_component_names, _, path = (
            self.collect_input(skip_collect_nothing=False)
        )
        if not input_component_hashes:
            logger.error("Targeted Ini Generation Aborted! No valid hashes provided.")
            return
    ...
    def start_extraction(self):
        ...
        if not input_component_hashes:
            logger.error("Frame Analysis Aborted! No valid hashes provided.")
            return
        if not extract_name:
            logger.error(
                "Frame Analysis Aborted! You must provided a name for the extracted model."
            )
            return
        if not (path / "log.txt").exists():
            logger.error(
                'Frame Analysis Aborted! Invalid frame analysis path: "%s".', str(path)
            )
            return
        ...
        extracted_components = frame_analysis.extract(
            ...
            reverse_shapekeys=(
                self.cfg.data.reverse_shapekeys_hsr
                if self.variant.value == "hsr"
                else self.cfg.data.reverse_shapekeys_zzz
                if self.variant.value == "zzz"
                else False
            ),
        )
        ...
        textures_component_idx = [
            idx
            for idx, extracted_component in enumerate(extracted_components)
            if extracted_component.options["collect_texture_data"]
            or extracted_component.options["collect_texture_hashes"]
        ]

        if len(textures_component_idx) > 0:
            self.state.lock_sidebar()
            self.parent.texture_picker.load(
                extract_name,
                extracted_components,
                textures_component_idx,
                finish_extraction_callback=self.finish_extraction,
            )
            self.parent.texture_picker.focus_set()
            self.update_idletasks()
            self.parent.show_texture_picker()
            self.update_idletasks()
            logger.info("Ready {:.3}s".format(time.time() - st))
        else:
            self.finish_extraction(extract_name, extracted_components)

    def finish_extraction(
        self, extract_name, extracted_components, collected_textures=None
    ):
        try:
            frame_analysis = self.state.get_var(State.K.FRAME_ANALYSIS)
            frame_analysis.export(
                extract_name,
                extracted_components,
                collected_textures,
                game=self.variant.value,
    ...
    def cancel_extraction(self):
        logger.warning("Frame Analysis Canceled.")
        self.state.unlock_sidebar()

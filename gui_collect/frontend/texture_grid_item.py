...
class TextureGridItem(tk.Canvas):
    def __init__(
        ...
        **kwargs,
        ...
        self.multi_texture_select_options = self.cfg.data.game[
            State.get_instance().active_page
    ...
    def create_canvas_widgets(self):
        outer_image_bg_tag = self.create_rectangle(
            0, 0, 272, 272, fill="#333", outline=""
        )
        inner_image_bg_tag = self.create_rectangle(
            8, 8, 264, 264, fill="#111", outline=""
        )
        self.inner_image_bg_tag = inner_image_bg_tag

        self.create_texture_hash_label()

        self.invis_event_target = self.create_rectangle(
            0, 0, 272, 272, fill="", outline=""
        )
        self.texture.async_read_format(callback=self.create_format_label)
        self.create_size_label()

        TextureManager.get_instance().get_image(
            self.texture, 256, callback=self.create_texture_image
        )

        do_popup = self.create_context_menu()

        self.tag_bind(
            self.invis_event_target,
            "<Enter>",
            add="+",
            func=lambda e: e.widget.itemconfig(outer_image_bg_tag, fill="#A00"),
        )
        self.tag_bind(
            self.invis_event_target,
            "<Leave>",
            add="+",
            func=lambda e: e.widget.itemconfig(outer_image_bg_tag, fill="#333"),
        )
        self.tag_bind(
            self.invis_event_target,
            "<Enter>",
            add="+",
            func=lambda e: e.widget.config(cursor="hand2"),
        )
        self.tag_bind(
            self.invis_event_target,
            "<Leave>",
            add="+",
            func=lambda e: e.widget.config(cursor=""),
        )
        self.tag_bind(
            self.invis_event_target, "<Button-1>", func=self.show_texture_type_picker
        )
    ...
    def create_texture_hash_label(self):
        if not self.texture.contamination:
            ...
            substrs = (
                "ps-t",
                str(self.texture.slot),
                "=",
                self.texture.contamination,
                "=",
                self.texture.hash,
            )
    ...
    def create_format_label(self):
        def helper_create_format_label():
            format_text_tag = self.create_text(
                11,
                10,
                anchor="nw",
                font=("Arial", 8, "bold"),
                fill="#e8eaed",
                text=self.texture._format,
            )
            format_text_bg_tag = self.create_rectangle(
                get_padded_bbox(self.bbox(format_text_tag), 2), fill="#222", outline=""
            )
    ...
    def create_size_label(self):
        ...
        size_text_bg_tag = self.create_rectangle(
            get_padded_bbox(self.bbox(size_text_tag), 2), fill="#222", outline=""
        )
    ...
    def create_resolution_label(self):
        ...
        res_text_bg_tag = self.create_rectangle(
            get_padded_bbox(self.bbox(res_text_tag), 2), fill="#222", outline=""
        )
    ...
    def show_texture_type_picker(self, *args):
        ...
        self.texture_type_frame = TextureTypeFrame(
            self.border_frame, self.handle_texture_type_click
        )
    ...
    def handle_texture_type_click(self, texture_type: str, *args):
        ...
        only_selected_draw_call = (
            self.multi_texture_select_options.only_selected_draw_call
        )
            ...
            for component_index, component in enumerate(self._texture_grid.components):
                for first_index in component.object_indices:
                    ...
                    if only_selected_draw_call:
                        draw_call_ids = [
                            self._texture_grid.frames[component_index][first_index][
                                "active_id"
                            ]
                        ]
                    else:
                    ...
                    for draw_call_id in draw_call_ids:
                        ...
                        for texture in component.draw_data[first_index][
                            draw_call_id
                        ].textures:
                            if texture.bleed and ignore_texture_bleed:
                                continue
                            if texture.hash == self.texture.hash:
                                self._texture_bar.component_textures[component_index][
                                    first_index
                                ].add_texture(texture, texture_type)
                                break
    ...
    def create_widgets(self):
        ...
        for texture_type in [
            ...
            texture_type_label.bind(
                "<Button-1>", partial(self.handle_texture_type_click, texture_type)
            )

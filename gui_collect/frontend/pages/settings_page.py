    ...
    def create_widgets(self):
        ...
        class _FlatButton(FlatButton):
            ...
            def enable(_self):
                ...
                _self.update_args(
                    new_bg=_self.accent,
                    new_hover_bg=_self.accent,
                    new_fg=self["bg"],
                    new_hover_fg=self["bg"],
                )

            def disable(_self):
                _self.is_active = False
                _self.update_args(
                    new_bg=darker(darker(darker(darker(_self.accent)))),
                    new_hover_bg=_self.accent,
                    new_fg=self["bg"],
                    new_hover_fg=self["bg"],
                )
...
class SettingsRouter(ScrollableFrame):
    def __init__(self, parent, initial_tab: tab_type, *args, **kwargs):
        super().__init__(
            parent, padx=0, pady=0, bg=parent["bg"], scrollbar_pad_x=(8, 0)
        )
    ...
    def create_widgets_general(self):
        ...
        self.create_checkbox(
            "Debugging Mode",
            cfg_key_path=["debugging"],
            callback=handle_logging_level,
            pady=(0, 8),
        )
        self.create_checkbox(
        ...
        connector_button = ConnectorButton(
            self.frame,
            enabled_cfg_path=["targeted_analysis_enabled"],
            bg="#222",
            padx=8,
            pady=8,
        )
    ...
    def create_multi_texture_select_options(
        self, accent_color: str, cfg_key_dir: list[str]
    ):
        lbl = tk.Label(
            self.frame,
            text="Multi-texture Select Options",
            anchor="w",
            font=("Arial", 18, "bold"),
            fg="#CCC",
            bg="#222",
        ...
        for cfg_key_path, text in [
            (
                cfg_key_dir + ["only_selected_draw_call"],
                "Only use textures from the selected draw call in the texture picker",
            ),
            (
                cfg_key_dir + ["ignore_texture_bleed"],
                "Ignore textures which where not explicitly bound to the shader within the draw call (texture bleed)",
            ),
        ]:
            self.create_checkbox(
                text, cfg_key_path=cfg_key_path, accent_color=accent_color, pady=(2, 0)
            )

        multi_texture_select_cfg = self.cfg.get_config_key_value(cfg_key_dir)
        draw_call_id_entry_validate_command = self.register(
            lambda new_text: (
                len(new_text) <= 5 and (new_text == "" or new_text.isdecimal())
            )
        )
        ...
        for key, text in [
            (
                "min_draw_call_id",
                "Minimum draw call index to automatically select textures from. Leave empty to use no minimum.",
            ),
            (
                "max_draw_call_id",
                "Maximum draw call index to automatically select textures from. Leave empty to use no maximum.",
            ),
        ]:
            ...
            string_var.trace_add(
                "write",
                callback=lambda a, b, c, _var=string_var: save_text_to_cfg(_var),
            )
            ...
            entry = tk.Entry(
                ...
                validate="key",
                validatecommand=(draw_call_id_entry_validate_command, "%P"),
            )
    ...
    def create_widgets_zzz(self):
        ...
        self.create_checkbox(
            "Reverse applied shapekeys. Requires `dump_cb` in d3dx.ini analyse_options or targeted analysis.",
            cfg_key_path=["reverse_shapekeys_zzz"],
            accent_color=accent_color,
            pady=(0, 8),
        )
        self.create_multi_texture_select_options(
            accent_color=accent_color,
            cfg_key_dir=["game", "zzz", "multi_texture_select_options"],
        )
    ...
    def create_widgets_hsr(self):
        ...
        self.create_checkbox(
            "Reverse applied shapekeys. Requires `dump_cb` in d3dx.ini analyse_options or targeted analysis.",
            cfg_key_path=["reverse_shapekeys_hsr"],
            accent_color=accent_color,
            pady=(0, 8),
        )
        self.create_multi_texture_select_options(
            accent_color=accent_color,
            cfg_key_dir=["game", "hsr", "multi_texture_select_options"],
        )
    ...
    def create_widgets_gi(self):
        ...
        self.create_multi_texture_select_options(
            accent_color=accent_color,
            cfg_key_dir=["game", "gi", "multi_texture_select_options"],
        )
    ...
    def create_widgets_hi3(self):
        ...
        self.create_multi_texture_select_options(
            accent_color=accent_color,
            cfg_key_dir=["game", "hi3", "multi_texture_select_options"],
        )
...
class ConnectorButton(tk.Frame):
    def __init__(self, parent, enabled_cfg_path: list[str], *args, **kwargs):
        ...
        self.cfg.register_config_update_handler(
            enabled_cfg_path, lambda is_active: self.refresh(is_active)
        )
        self.bind(
            "<Destroy>",
            lambda _: self.cfg.deregister_config_update_handler(enabled_cfg_path),
        )
    ...
    def create_widgets(self):
        ...
        self.lbl_main = tk.Label(
            self, text="Install connector .ini", font=("Arial", 16, "bold"), **kwargs
        )
        self.lbl_extra = tk.Label(
            self,
            text="Required for gui_collect based targeted analysis",
            font=("Arial", 10),
            **kwargs,
        )

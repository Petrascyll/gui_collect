...
def adjust_luminance(hex_code: str, amount: float):
    ...
    hls_code = (hls_code[0], clamp(hls_code[1] + amount, 0, 1), hls_code[2])
...
def rgb_to_hex(rgb_code: tuple[str]):
    ...
    # return hex_code
    return "#" + "".join([
        hex(int(c * 255)).removeprefix("0x").zfill(2) for c in rgb_code
    ])
...
APP_STYLE = {
    ...
    "image": {
        "default": {"border": COLOR["Black.2"]},
        "active": {"border": COLOR["Aquamarine"]},
    },
}
...
DEFAULT_CANVAS_STYLE = {"highlightthickness": 0, "relief": "flat"}

...
StructuredBuffer<TextPositionParameters> pos_params : register(t115);
...
void main()
{
	...
	float space_x = 0;
	float max_x = -1;
	float rect_width = pos_params[0].rect.z - pos_params[0].rect.x;
...
void draw_outline_rectangle(float4 r, inout TriangleStream<gs2ps> ostream)
{
	...
	output.pos = float4(r.x - pos_params[0].border.x, r.y + pos_params[0].border.y, -1, 1);
	ostream.Append(output);
	output.pos = float4(r.z + pos_params[0].border.x, r.y + pos_params[0].border.y, -1, 1);
	ostream.Append(output);
	output.pos = float4(r.x - pos_params[0].border.x, r.w - pos_params[0].border.y, -1, 1);
	ostream.Append(output);
	output.pos = float4(r.z + pos_params[0].border.x, r.w - pos_params[0].border.y, -1, 1);
	ostream.Append(output);
...
void main(point vs2gs input[1], inout TriangleStream<gs2ps> ostream)
{
	...
	if (pos_params[0].h_anchor == 1) { // Anchor Left
		r.xz = float2(r.x, r.x + textpos[0].pos.x);
		if (pos_params[0].h_align)
			text_shift.x = (r.z - pos_params[0].rect.z);
	} else if (pos_params[0].h_anchor == 2) { // Anchor Center
		r.xz = float2(center.x - textpos[0].pos.x/2, center.x + textpos[0].pos.x/2);
		if (pos_params[0].h_align)
			text_shift.x = (r.z - pos_params[0].rect.z) * 2;
	} else if (pos_params[0].h_anchor == 3) { // Anchor Right
		r.xz = float2(r.z - textpos[0].pos.x, r.z);
		if (pos_params[0].h_align)
			text_shift.x = (pos_params[0].rect.x - r.x);
	}

	if (pos_params[0].h_align == 1)
		text_shift.x /= 2;

	if (pos_params[0].v_anchor == 1) // Anchor Top
		r.yw = float2(r.y, r.y + textpos[0].pos.y);
	else if (pos_params[0].v_anchor == 2) // Anchor center
		r.yw = float2(center.y - textpos[0].pos.y/2, center.y + textpos[0].pos.y/2);
	else if (pos_params[0].v_anchor == 3) // Anchor Bottom
